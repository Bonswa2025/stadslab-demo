...
const variantClass = {
  ...
  link: 'bg-transparent text-blue-600 underline border-transparent'
} as const;
const sizeClass = { sm:'px-2 py-1 text-xs', md:'px-3 py-1.5 text-sm', lg:'px-4 py-2 text-base' } as const;
export function Button({ className='', variant='default', size='md', ...props }: ButtonProps){
  return <button className={`inline-flex items-center justify-center rounded-md border shadow-sm ${variantClass[variant]} ${sizeClass[size]} ${className}`} {...props} />;
}
